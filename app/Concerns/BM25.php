...
class BM25
{
  private static function IDF($N, $nq_i)
  {
    /*
  ...
  public static function score($Q, $docCollection, $stemmer = null, $k = 1.2, $b = 0.75, $paramaterCheck = TRUE, $warningCheck = TRUE)
  {
    /*
    ...
    */
    ...
    for ($documentCount = 0; $documentCount < $N; $documentCount++) {
      ...
      // Tokenise the document and extract stems
      $tokens = explode(" ", str_replace(array('.', ',', "\t", '!', '\"', '£', '$', '%', '^', '&', '*', '(', ')', '-', '_', '=', '{', '}', '[', ']', ';', ':', '@', '~', '<', '>', '?', '/', '\\', '\`', '¬', "\n", "\r"), ' ', $docCollection[$documentCount]));
      if (isset($stemmer)) {
        foreach ($tokens as $token) {
          $TokeniseddocCollection[$documentCount][] = $stemmer->stem($token);
        }
      } else {
        $TokeniseddocCollection[$documentCount] = $tokens;
      }
    ...
    for ($documentCount = 0; $documentCount < $N; $documentCount++) {
      ...
      for ($queryCount = 0; $queryCount < $NumOfQueries; $queryCount++) {
        ...
        if ($Q[$queryCount] != "") {
          $individualScore += self::IDF($N, $nq_i[$queryCount]) * $queryFrequencyPerDocument[$documentCount][$queryCount] * ($k + 1) / ($queryFrequencyPerDocument[$documentCount][$queryCount] + $k * (1 - $b + $b * $wordsindoc[$documentCount] / $avgld));
        }
      }
