...
class CustomArabicStemmer implements Stemmer
{
    ...
    public static function stem($word)
    {
        ...
        $stem = preg_replace($patterns, $replacements, $word);

        if (mb_strlen($stem, 'UTF-8') <= 3) return $stem;

        $nounStem = self::roughStem(
            $stem, self::$_nounMay, self::$_nounPre, self::$_nounPost,
            self::$_nounMaxPre, self::$_nounMaxPost, self::$_nounMinStem
        );
        $verbStem = self::roughStem(
            $stem, self::$_verbMay, self::$_verbPre, self::$_verbPost,
